-- ── Manual (owner-created) bookings: email optional ──────────────────────────
-- The owner can now create bookings from the widget manager on behalf of a
-- client who calls in. Such a client often has no email to give, so email stops
-- being mandatory: the booking requires only a customer name (phone is enforced
-- by the web/manual /book route, but stays optional in the RPC so the MCP/AI
-- path is unchanged). A blank email is stored as '' — the column stays NOT NULL,
-- and `dispatchBookingMessage` already skips the email channel when it's empty.
--
-- This is a straight re-issue of the address-aware create_booking_tx
-- (20260819140000) with a single behavioral change: the guard no longer requires
-- an email, and each insert coalesces a null email to ''. Everything else is
-- byte-for-byte the prior body.
--
-- NOTE (RPC drift): if prod's create_booking_tx has diverged from migrations,
-- dump `pg_get_functiondef` before pushing to prod and fold this one-line change
-- into the live body rather than blindly replacing it.
