...
-- A single booking can now cover several services at once. Their durations and
-- prices add up: the reserved slot spans the summed duration and the charged
-- amount is the summed price.
--
-- Backward compatibility:
--   • The existing denormalized columns stay the source of truth for every
--     current consumer (notifications, calendar, manage page, revenue stats):
--       - service_id    → the FIRST (primary) selected service
--       - service_name  → the selected services joined with " + "
--       - duration_min  → the SUM of durations (reserves the full slot)
--       - price_cents   → the SUM of prices
--   • A new nullable `services` jsonb column captures the per-service breakdown
--     (array of {service_id, name, duration_min, price_cents}). It is populated
--     only when more than one service is booked; single-service bookings (incl.
--     the MCP/AI path) leave it null and behave exactly as before.
--   • create_booking_tx gains a trailing `p_service_ids text[]` (default null).
--     Callers that don't pass it (e.g. the MCP edge function) fall through to
--     the unchanged single-service path via p_service_id.

alter table public.widget_bookings
  add column if not exists services jsonb;

comment on column public.widget_bookings.services is
  'Per-service breakdown for multi-service bookings: array of {service_id, name, duration_min, price_cents}. Null ⇒ single-service booking (see service_id/service_name).';

-- ── RPC: create_booking_tx (multi-service aware) ─────────────────────────────
-- Signature changes (adds p_service_ids), so drop the old overload first.
drop function if exists public.create_booking_tx(uuid, text, timestamptz, text, text, text, text, uuid, text, text);
...
revoke all on function public.create_booking_tx(uuid, text, timestamptz, text, text, text, text, uuid, text, text, text[]) from public;
grant execute on function public.create_booking_tx(uuid, text, timestamptz, text, text, text, text, uuid, text, text, text[]) to service_role;
