-- ── Customer mailing/service address on bookings ─────────────────────────────
-- Some businesses (mobile / at-home services) need the customer's address at
-- booking time. Whether the public widget shows the field, and whether it is
-- required, is owner-controlled per widget instance via the calendar config
-- JSON (config.collect_address / config.address_required) — no schema for those,
-- exactly like config.show_prices.
--
-- Here we only add the storage column and thread it through create_booking_tx:
--   • A new nullable `customer_address text` column on widget_bookings.
--   • create_booking_tx gains a trailing `p_customer_address text` (default null).
--     Appended at the very end so existing positional callers (e.g. the MCP edge
--     function, which never passes it) fall through unchanged and store null.

alter table public.widget_bookings
  add column if not exists customer_address text;

comment on column public.widget_bookings.customer_address is
  'Optional customer mailing/service address captured at booking time. Shown/required per the owner''s config.collect_address / config.address_required (JSON, no column).';

-- ── RPC: create_booking_tx (address aware) ───────────────────────────────────
-- Signature changes (adds a trailing p_customer_address), so drop the old
-- overload first.
drop function if exists public.create_booking_tx(uuid, text, timestamptz, text, text, text, text, uuid, text, text, text[]);
  ...
  if jsonb_array_length(v_staff_all) = 0 then
    begin
      insert into public.widget_bookings (
        ...
        p_starts_at, v_ends_at, p_customer_name, p_customer_email, p_customer_phone, p_customer_address, p_notes,
